...
void ADoor::CloseDoor(const float DeltaTime)
{
	...
	case EDoorBehaviourType::DoorType_Vertical:
		...
		break;
	case EDoorBehaviourType::DoorType_Horizontal:
		if(SlideOnRelativeY){CurrentPositionAddon.Y =  FMath:: Lerp(CurrentPositionAddon.Y,0,OpenCloseSpeed*DeltaTime);}
		else{CurrentPositionAddon.X =  FMath:: Lerp(CurrentPositionAddon.X,0,OpenCloseSpeed*DeltaTime);}		
		DoorMesh->SetRelativeLocation(CurrentPositionAddon);
		break;
	default: ;
	}
